...
async function generateFinalExcel() {
    ...
    const colorOlive = 'FF8C845D'; 
    const colorGold = 'FFD4AF37'; 
    const colorHeader = 'FFF2F1ED';
    const colorBorder = 'FFD1D1D1';

    // ==========================================
    // SHEET 1: CONFIGURAÇÕES DA MAYCLICK
    // ==========================================
    const sheetConfig = workbook.addWorksheet('Pacotes e Configurações');
    
    // Configuração da Empresa
    sheetConfig.getCell('I2').value = 'DADOS DA MAYCLICK (EDITÁVEL)';
    ...
    const bizData = [
        ...
        ['Validade Orçamento (Dias)', 5],
        ['Forma de Pagamento', '50% no ato da contratação e 50% no dia do evento.'],
        ['Cartão de Crédito', 'Em até 12x no crédito (juros a consultar).']
    ];
    
    bizData.forEach((row, i) => {
        sheetConfig.getCell(`I${i+4}`).value = row[0];
        sheetConfig.getCell(`I${i+4}`).font = { bold: true, size: 9 };
    ...
    sheetConfig.columns = [
        ...
        { header: 'VALOR BASE', key: 'price', width: 15 },
    ...
    sheetGen.getCell('G2').value = '0001'; 
    ...
    const calcFields = [
        ...
        ['Valor da Hora (R$):', 'D11', 'E11'],
        ['Deslocamento (R$):', 'B12', 'C12'],
        ['Itens Extras (R$):', 'D12', 'E12'],
        ['Desconto (R$):', 'B13', 'C13']
    ];
    ...
    // ==========================================
    // SHEET 3: ORÇAMENTO FINAL (PDF READY)
    // ==========================================
    ...
    header.value = 'PROPOSTA COMERCIAL - ORÇAMENTO FOTOGRÁFICO';
    header.font = { size: 18, bold: true, color: { argb: colorOlive }, name: 'Arial Black' };
    ...
    sheetFinal.getCell('A4').font = { size: 12, bold: true };
    
    sheetFinal.getCell('E4').value = 'Mayclick Photography'; 
    sheetFinal.getCell('E4').font = { bold: true, color: { argb: colorOlive }, size: 12 };
    sheetFinal.getCell('E4').alignment = { horizontal: 'right' };

    // Info Grid
    sheetFinal.getCell('A6').value = 'DADOS DO PRESTADOR:';
    sheetFinal.getCell('A6').font = { bold: true, size: 8, color: { argb: colorOlive } };
    sheetFinal.getCell('A7').value = { formula: `CONCATENATE("WhatsApp: ", 'Pacotes e Configurações'!J5, " | CNPJ: ", 'Pacotes e Configurações'!J6)` };
    sheetFinal.getCell('A8').value = { formula: `'Pacotes e Configurações'!J7` }; 
    sheetFinal.getCell('A7').font = { size: 8 };
    sheetFinal.getCell('A8').font = { size: 8 };

    sheetFinal.getCell('E6').value = 'PREPARADO PARA:';
    sheetFinal.getCell('E6').font = { bold: true, size: 8, color: { argb: colorOlive } };
    sheetFinal.getCell('E6').alignment = { horizontal: 'right' };
    sheetFinal.getCell('E7').value = { formula: `'Gerador de Orçamento'!C5` };
    sheetFinal.getCell('E7').alignment = { horizontal: 'right' };
    sheetFinal.getCell('E8').value = { formula: `CONCATENATE('Gerador de Orçamento'!F7, " | Data: ", TEXT('Gerador de Orçamento'!F5, "dd/mm/yyyy"))` };
    sheetFinal.getCell('E8').alignment = { horizontal: 'right' };

    // Presentation
    sheetFinal.mergeCells('A10:G12');
    const intro = sheetFinal.getCell('A10');
    intro.value = 'A Mayclick Photography atua há 4 anos registrando eventos sociais com cuidado, sensibilidade e compromisso. Cada evento é tratado como uma memória única, e nosso objetivo é entregar uma experiência segura, organizada e especial para cada cliente.';
    intro.font = { size: 9, color: { argb: colorGraphite } };
    intro.alignment = { wrapText: true, vertical: 'middle', horizontal: 'center' };

    // Table Header
    sheetFinal.getRow(14).values = ['SERVIÇO SELECIONADO', '', 'DETALHAMENTO E ENTREGAS', '', '', '', 'INVESTIMENTO'];
    ...
    sheetFinal.getCell('C15').font = { size: 8 };
    ...
    sheetFinal.getCell('G15').font = { bold: true, size: 10, color: { argb: colorGold } };

    // Additional Info
    sheetFinal.getCell('A19').value = 'CONDIÇÕES DE PAGAMENTO:';
    sheetFinal.getCell('A19').font = { bold: true, size: 9, color: { argb: colorOlive } };
    sheetFinal.mergeCells('A20:G20');
    sheetFinal.getCell('A20').value = { formula: `CONCATENATE('Pacotes e Configurações'!J10, " ", 'Pacotes e Configurações'!J11)` };
    sheetFinal.getCell('A20').font = { size: 9 };

    sheetFinal.getCell('A22').value = 'OBSERVAÇÕES E TERMOS:';
    sheetFinal.getCell('A22').font = { bold: true, size: 9, color: { argb: colorOlive } };
    sheetFinal.mergeCells('A23:G24');
    sheetFinal.getCell('A23').value = { formula: `CONCATENATE("Este orçamento é válido por ", 'Pacotes e Configurações'!J9, " dias. Após a aprovação, o contrato é enviado para assinatura digital via plataforma ZapSign, garantindo total segurança e validade jurídica para ambas as partes.")` };
    sheetFinal.getCell('A23').font = { size: 8 };
    sheetFinal.getCell('A23').alignment = { wrapText: true };

    // Footer
    sheetFinal.mergeCells('A26:G26');
    sheetFinal.getCell('A26').value = 'Mayclick Photography | @mayclick_fotos | www.mayfotosefilmagens.com.br';
    sheetFinal.getCell('A26').font = { size: 8, italic: true, color: { argb: colorOlive } };
    sheetFinal.getCell('A26').alignment = { horizontal: 'center' };

    // Save
    const dirPath = path.join(__dirname, '..', 'deliverables', 'ENTREGA_FINAL_MAYCLICK');
    const outputPath = path.join(dirPath, 'gerador-orcamentos-mayclick.xlsx');
    await workbook.xlsx.writeFile(outputPath);
    console.log('Planilha Final Mayclick gerada com sucesso!');
}

generateFinalExcel().catch(err => console.error(err));
